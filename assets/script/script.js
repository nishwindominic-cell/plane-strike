...
// Open mystery box in game (when player clicks on it) - FIXED to safely resume game
function openMysteryBox() {
    ...
    // Animate and remove box - FIXED: Resume game after box closes
    setTimeout(() => {
        container.style.display = 'none';
        box.classList.remove('opening');
        box.classList.remove('hover');

        // Resume game if player was in mission
...
// Save all pilot information and hide login page - FIXED: Ad only after validation
function savePilotInfo() {
    ...
    // Show ad only after valid input - FIXED POSITION
    if (window.Android) {
        Android.showMainAd();
    }
}
...
// Called when Android gives reward after watching rewarded ad
...
// Backup fix when app comes back from ad / focus loss
...
// ===== ANDROID AD CLOSE / RESUME FIX =====

// Called by Android when interstitial ad is closed
window.onInterstitialClosed = function () {
    console.log("Interstitial ad closed");

    // Remove accidental pause caused by ad focus loss
    isPaused = false;

    // Hide pause menu if it got stuck open
    const pauseMenu = document.getElementById('pause-menu');
    if (pauseMenu) {
        pauseMenu.style.display = 'none';
    }

    // Keep game over screen if player is dead
    if (player && player.dead) {
        showGameOverMenu();
    }
};

// Called by Android when rewarded ad gives reward
function onRewardEarned(rewardType, rewardAmount) {
    console.log("Reward earned:", rewardType, rewardAmount);
    ...
    // Continue your existing revive logic
    if (typeof window.onRewardAdCompleted === "function") {
        window.onRewardAdCompleted();
    }
}

// Optional safety fix: when app/tab comes back into focus
window.addEventListener("focus", function () {
    const pauseMenu = document.getElementById('pause-menu');

    // If game is active and player is alive, unpause
